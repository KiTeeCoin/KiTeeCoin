...
# Copyright (c) 2021 The KiTeecoin Core developers
...
VERSION=
commit=false
url=https://github.com/KiTeecoin/KiTeecoin
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the KiTeecoin, gitian-builder, gitian.sigs, and KiTeecoin-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/KiTeecoin/KiTeecoin
...
then
	...
	echo ""
	./bin/gverify -v -d ../gitian.sigs/ -r ${VERSION}-win-unsigned ../KiTeecoin/contrib/gitian-descriptors/gitian-win.yml
	# Mac OSX	
	echo ""
	echo "Verifying v${VERSION} Mac OSX"
	echo ""	
	./bin/gverify -v -d ../gitian.sigs/ -r ${VERSION}-osx-unsigned ../KiTeecoin/contrib/gitian-descriptors/gitian-osx.yml
	# Signed Windows
	echo ""
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../KiTeecoin/contrib/gitian-descriptors/gitian-win-signer.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-win-signed --destination ../gitian.sigs/ ../KiTeecoin/contrib/gitian-descriptors/gitian-win-signer.yml
	    mv build/out/KiTeecoin-*win64-setup.exe ../KiTeecoin-binaries/${VERSION}
	    mv build/out/KiTeecoin-*win32-setup.exe ../KiTeecoin-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../KiTeecoin/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-osx-signed --destination ../gitian.sigs/ ../KiTeecoin/contrib/gitian-descriptors/gitian-osx-signer.yml
	    mv build/out/KiTeecoin-osx-signed.dmg ../KiTeecoin-binaries/${VERSION}/KiTeecoin-${VERSION}-osx.dmg
	fi
	popd
