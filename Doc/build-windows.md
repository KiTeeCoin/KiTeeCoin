...
Below are some notes on how to build KiTeeCoin Core for Windows.
...
Note that for WSL v1 the KiTeeCoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/KiTeeCoin, AND not under, for example, /mnt/d/KiTeeCoin. 
...
way. This will install to `c:\workspace\KiTeeCoin`, for example:

    make install DESTDIR=/mnt/c/workspace/KiTeeCoin
