# KiTeecoin Core 1.8.1

KiTeecoin Core 1.8.1 is primarily a bugfix release, bringing KiTeecoin Core in
line with Bitcoin 0.9.3. KiTeecoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by KiTeecoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 KiTeecoin Core clients
* Use HTTPS with KiTeecoin.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/KiTeecoin/KiTeecoin/issues/134).
...
This only affects values significantly below 1 KITee, however merchants may wish to round to
the nearest KiTee on all transactions for now.
