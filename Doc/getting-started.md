...
This tutorial will help you to go through the basics to use KiTeeCoin Core after you completed the [installation instructions](/INSTALL.md). You now have `dogecoind` or `dogecoin-qt` executables available to run a node, and `dogecoin-cli`/`dogecoin-tx` tools to help you transact DOGE.
...
> ```console
> shibetoshi:~$ /path/to/KiTeeCoin-cli [arguments ...]
> ```

### Table of contents

1. [Starting a KiTeeCoin node](#starting-a-KiTeeCoin-node)
...
## Starting a KiTeeCoin node

To start your node, you can run an headless server using `KiTeeCoind`:
```console
shibetoshi:~$ KiTeeCoind -daemon
```

Or you can use the Graphical User Interface (GUI), `KiTeeCoin-qt`:
```console
shibetoshi:~$ KiTeeCoin-qt
```
...
> **Note:** The rest of this guide assumes the use of an headless node. The RPC server is not exposed with `KiTeeCoin-qt` until you activate the `-server` option as a startup argument, but inside the GUI application, you can use all the commands explored below (without `dogecoin-cli`) by going to `Help -> Debug window` and inside the popup window selecting the tab `Console`.

## Introduction to the JSON-RPC API

KiTeeCoin Core exposes a JSON-RPC interface that allows you to request information about the network, blockchain and individual transactions, send transactions to the networks and manage your wallet.

The KiTeeCoin Core installation provides the `KiTeeCoin-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `22555`, so that other tools and libraries can interact with it.

To have an overview of the available commands, use the `help` command:

```console
#List all commands
shibetoshi:~$ KiTeeCoin-cli help

#Get help for a specific command
shibetoshi:~$ KiTeeCoin-cli help COMMAND
```
...
```console
shibetoshi:~$ KiTeeCoin-cli getnewaddress
...
```console
shibetoshi:~$ KiTeeCoin-cli backupwallet /path/of/wallet/backup
```

**Tip:** KiTeeCoin addresses start with the letter `D`.

You now have two wallet addresses to share with other people to receive KITEE! Consider avoiding [address reuse](https://en.bitcoin.it/wiki/Address_reuse) for anonymity and security reasons.
...
```console
#Syntax
shibetoshi:~$ KiTeeCoin-cli getbalance "*" minconf
```
...
```console
#Syntax
shibetoshi:~$ KiTeeCoin-cli sendtoaddress address amount

#Example
shibetoshi:~$ KiTeeCoin-cli sendtoaddress nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh 420
```
...
```console
#Syntax
shibetoshi:~$ KiTeeCoin-cli listunspent minconf maxconf '["address", ...]'

#Example
shibetoshi:~$ KiTeeCoin-cli listunspent 1 9999999 '["nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P"]'
[
  {
...
shibetoshi:~$ KiTeeCoin-cli createrawtransaction "$utxos_to_use" '{"address":amount, ...}'
...
shibetoshi:~$ KiTeeCoin-cli createrawtransaction "$utxos_to_use" '{"nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh":69, "nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P": 30.999}'
...
> **Tip:** The transaction returned is encoded in hexadecimal encoding. You can use `KiTeeCoin-cli decoderawtransaction` or `KiTeeCoin-tx -json` to convert the content to JSON format.

##### signrawtransaction

Before sending a transaction, it must be signed by the private key that the address was derived from. KiTeeCoin Core will automatically use the correct private key when spending UTXO known to the wallet.

```console
#Syntax
shibetoshi:~$ KiTeeCoin-cli signrawtransaction encoded_transaction

#Example
shibetoshi:~$ KiTeeCoin-cli signrawtransaction "01000000013324faf8f03695261611669f5bdd93b68a86cf02db12dc46642ed50666ed69b80000000000ffffffff0200a5459b010000001976a91418a89ee36293f15c4db4c01173babd579243161188ac60b8c4b8000000001976a914c6977da37560e1432c2e14e16952981a4c272cac88ac00000000"
{
...
```console
#Syntax
shibetoshi:~$ KiTeeCoin-cli sendrawtransaction signed_transaction

#Example
shibetoshi:~$ KiTeeCoin-cli sendrawtransaction 
...
shibetoshi:~$ KiTeeCoin-cli getblockhash 69
3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b

#Get block data
shibetoshi:~$ KiTeeCoin-cli getblock 3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b
{
...
```console
#Syntax
shibetoshi:~$ KiTeeCoin-cli getrawtransaction txid verbose

#Example
shibetoshi:~$ KiTeeCoin-cli getrawtransaction 695ce4208fa7a87ef9e99805b0910dc129058ecdceb5cef7e25f71dcdc7936db 1
{
...
Using `KiTeeCoind -help` will display all available configuration parameters that can be added as arguments:

**Command example :**
```console
shibetoshi:~$ KiTeeCoind -daemon -paytxfee=0.01 -sendfreetransactions=1 -maxconnections=150
```

Configuration can be persisted by creating a `KiTeeCoin.conf` file. Create it in the directory defined with the `datadir` setting, `$HOME/.dogecoin` by default, or specify the file location with `-conf`.
...
maxconnections=150
```
You can see a more concrete example [here](/contrib/debian/examples/KiTeeCoin.conf).

### Mainnet, testnet and regtest

When trying out new things, for example to test your application that interacts with the KiTeeCoin chain, it is recommended to not use the main Dogecoin network. Multiple networks are built-in for this purpose.
...
When not specifying any network, *Mainnet* is the network used by default. To enable *testnet*, use the `KiTeeCoind -testnet`.

To enable *regtest*, use the `-regtest` option.

> **Tip:** Remember to specify the network when you want to use `KiTeeCoin-cli`.

### Data directory

The data directory is the location where KiTeeCoin Core files are stored, including the wallet, log files and blocks. You can modify the location with the `-datadir` setting.

**Default location :**

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.KiTeeCoin`
macOS    | `$HOME/Library/Application Support/KiTeeCoin`
Windows  | `%APPDATA%\KiTeeCoin`

You may need to specify `-datadir` also when using `KiTeeCoin-cli`.

See the [full documentation on file system](files.md) for more information.

### RPC credentials

Authentication is required to interact with the RPC interface. When no credentials are provided, KiTeeCoin uses a [random cookie](https://bitcoin.org/en/release/v0.12.0#rpc-random-cookie-rpc-authentication) that gets generated when the software is launched. It's possible to define your own credentials using `rpcuser` and `rpcpassword` parameters.
...
Running KiTeeCoin Core can require a lot of memory, so in some situations it may be necessary to optimize its usage. You can find more information about reducing the memory footprint in the [related guide](reduce-memory.md).

### Troubleshooting

By default, KiTeeCoin Core keeps detailed logs in the `debug.log` file, located in the `datadir`. Alternatively, the `-printtoconsole` parameter displays the log interactively to the terminal instead.
